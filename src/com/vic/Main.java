...
public class Main {
    ...
    public static void main(String[] args) {
         ...
         * 进入登陆模块（参数为实例teachers），输入用户名密码，错误则重新输入
         * 进入永真循环，检索输入后进行switch判断：
         * 检索输入模块checkDemand（参数为stage）：定义信号变量int checkCode，初始为404
         * 如果检索到“切换为”，则判断后续并返回为小学（checkCode=8848）/初中（8849）/高中（8850），若不符合则重新输入
         * 如果检索到-1，则赋值checkCode后返回，
         * 如果检索到10-30的整数，则返回作为出题数量
         * 定义外部变量checkCode接收输入模块checkDemand返回的int值
         * 开始switch判断：
         * case -1：重新进入登录模块
         * case 8848：更新当前用户的stage为8848
         * case 8849：更新当前用户的stage为8849
         * case 8850：更新当前用户的stage为8850
